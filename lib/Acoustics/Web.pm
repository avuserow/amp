...
sub paged_search {
	...
	my $sth = $self->acoustics->db->prepare('SELECT * FROM songs WHERE online = 1 AND (album LIKE ? OR artist LIKE ? OR title LIKE ? OR path LIKE ?) ORDER BY album, disc, track LIMIT ? OFFSET ?');
